...
def load_df_data_v1():
    global  df
    conn=mariadb.connect(host='localhost',user='root',passwd='9bhtj93g',db='cnyes_news')
    cursor=conn.cursor()
    cursor.execute('select `item_id`,`category`,`content`,`date`,`title`,`link`,`top_key_freq`,`sentiment` from cnyes;')
    items_id=[]
    categories=[]
    contents=[]
    dates=[]
    ...
    for item_id,category,content,date,title,link,top_key_freq,sentiment in cursor:
        ...
        titles.append(title)
        links.append(link)
        top_key_freqs.append(top_key_freq)
    ...
    data=zip(items_id,categories,contents,dates,titles,links,top_key_freqs,sentiments)
    df=pd.DataFrame(list(data), columns=['item_id','category','content','date','title','link','top_key_freq','sentiment'])
    df.date=df.date.apply(lambda row: str(row))
    df.sentiment=df.sentiment.apply(lambda row :float(row))
...
def filter_df_via_content(query_keywords, cond, cate, weeks):
    ...
    # start date
    start_date_delta = (datetime.strptime(end_date, '%Y-%m-%d %H:%M:%S').date() - timedelta(weeks=weeks)).strftime('%Y-%m-%d')
