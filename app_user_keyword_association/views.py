...
def load_df_data_v1():
    ...
    for i in range(0,(len(items_id))):
        ca=[]
        for j in (re.findall(r'\[.*?,.*?\]',top_key_freqs[i])):
            dd=j.replace('[','[\'').replace(']','\']').replace(',','\',\'').replace(' ','')
            
            try:
                freq=int(list(eval(dd))[1])
                word=str(list(eval(dd))[0])
                ca.append(list((word,freq)))
            except:
                pass
        alls.append(ca)

    data=zip(items_id,categories,contents,dates,titles,links,alls,sentiments)
